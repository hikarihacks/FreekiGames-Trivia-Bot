...
def pickTrivia(i):
	triviaNameList = ["Adventuring", "Kingisle", "Conjuring", "Magical", "Marleybone", "Mystical", "Spellbinding", "Spells", "Wizard City", "Zafaria"]

	pick_trivia(triviaNameList[i])
	time.sleep(3)
	change_trivia()
	time.sleep(3)
...
def main(name): #Testing purpose only!!
	i = 0
	# for all of the positions in triviaNameList:
	for i in range(len(triviaNameList)-1):
		# If name equals the word stored in the position of the list being checked...
		if (name == triviaNameList[i]):
			pickTrivia(i)
	"""
	# I set it up this way first. Then was able to simplify it even more.
	# The code above this commented out spot is the most optimized I could come up with.
    if name=="Adventuring":
        # pickTrivia(i)
		pickTrivia(0) # Adventuring
		
    elif name=="Kingsisle":
        pickTrivia(1) # Kingsisle

    elif name=="Conjuring":
        pickTrivia(2) # Conjuring

    elif name=="Magical":
        pickTrivia(3) # Magical

    elif name=="Marleybone":
        pickTrivia(4) # Marleybone

    elif name=="Mystical":
        pickTrivia(5) # Mystical

    elif name=="Spellbinding":
        pickTrivia(6) # Spellbinding

    elif name=="Spells":
        pickTrivia(7) # Spells

    elif name=="Wizard City":
        pickTrivia(8) # Wizard City
		
    elif name=="Zafaria":
        pickTrivia(9) # Zafaria
	"""